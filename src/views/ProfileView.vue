...
import { Check, DocumentChecked, MagicStick, User } from '@element-plus/icons-vue'
import { interviewApi } from '../utils/interview'

const loading = ref(true)
const saving = ref(false)
const focusOptions = ['项目深挖', '前端基础', 'Vue / React', '性能优化', 'Agent / RAG', '系统设计', '行为面试']
      ...
      <div class="form-stack">
        <article class="io-panel form-panel">
          ...
          <div class="two-columns">
            <label><span>目标方向</span><el-input v-model="workspace.profile.targetRole" maxlength="80" placeholder="例如：前端开发 / AI 应用工程" /></label>
          ...
          <div class="focus-field"><span>重点训练领域</span><div class="focus-tags"><button v-for="item in focusOptions" :key="item" :class="workspace.profile.focusAreas.includes(item) && 'active'" @click="toggleFocus(item)"><el-icon v-if="workspace.profile.focusAreas.includes(item)"><Check /></el-icon>{{ item }}</button></div></div>
        </article>
        ...
        <article class="io-panel form-panel">
          ...
          <div class="two-columns">
            ...
            <label><span>岗位名称</span><el-input v-model="workspace.target.jobTitle" maxlength="80" placeholder="例如：前端开发工程师" /></label>
...
.profile-grid { display: grid; grid-template-columns: minmax(0,1fr) 280px; gap: 17px; margin-top: 17px; }.form-stack,.guidance-stack { display: grid; align-content: start; gap: 15px; }
...
@media(max-width:1050px){.profile-grid{grid-template-columns:1fr}.guidance-stack{grid-template-columns:repeat(3,1fr)}}
