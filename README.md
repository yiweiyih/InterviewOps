...
Agent 任务与 SSE 连接相互独立：前端先通过 POST /api/agent/runs 创建一次任务并获得 runId，再通过 GET /api/agent/runs/:runId/events 订阅事件。每条事件都带有单调递增的 seq，断线后前端携带 Last-Event-ID 重新订阅，服务端只补发缺失事件，前端也按序号丢弃重复事件，因此不会重新执行 Agent 或重复追加消息。

创建任务使用客户端生成的 Idempotency-Key，即使创建响应丢失后重试，也只会返回原来的 runId。服务端当前用内存 Map 暂存事件，完成后保留 30 分钟，并每 15 秒发送心跳；用户主动停止时调用独立取消接口，中断服务端模型流。生产环境可将同一模型迁移到 Redis，以支持多实例与进程重启恢复。
